...
export default function Home() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const existingDocuments: Document[] = documentsResponse.documents.map((doc: {
          id: string;
          filename: string;
          size: number;
          upload_date: string;
  ...
  const handleSendMessage = useCallback(async (message: string) => {
    ...
    try {
      ...
      console.log('📄 Available documents:', documents.map(doc => doc.id));
      
      const result = await api.sendMessage(message, documents.map(doc => doc.id));
      ...
      if (error instanceof Error) {
        ...
        const axiosError = error as { response?: { data?: { detail?: string } }; message?: string };
        errorMessage = `Error: ${axiosError.response?.data?.detail || axiosError.message || 'Request failed'}`;
      }
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8 text-center">
          ...
          <p className="text-muted-foreground">
            Upload PDF documents and chat with AI to get insights, summaries, and answers
          </p>
          <div className="mt-4">
            {backendStatus === 'checking' && (
              <div className="text-yellow-600">🔍 Checking backend connection...</div>
            )}
            {backendStatus === 'connected' && (
              <div className="text-green-600">✅ Backend connected</div>
            )}
            {backendStatus === 'error' && (
              <div className="text-red-600">❌ Backend connection failed. Please ensure the backend is running on http://localhost:8000</div>
            )}
          </div>
        </div>
